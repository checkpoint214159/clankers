...
// Combined arm + adapter + LEAP hand, generated by `uv run clankers-build-urdf`.
// It spans two mesh packages: the arm meshes are referenced in their existing arm02
// package rather than duplicated, so this profile needs a package map, not a single entry.
const CLANKERS_PACKAGE_PATH = `${import.meta.env.BASE_URL}resources/clankers`;
const CLANKERS_URDF_PATH = `${CLANKERS_PACKAGE_PATH}/urdf/clankers_combined.urdf`;
...
const PROFILE_MODELS = {
  ...
  clankers: {
    packagePath: CLANKERS_PACKAGE_PATH,
    urdfPath: CLANKERS_URDF_PATH,
    packageName: 'clankers_combined',
    packages: {
      clankers_combined: CLANKERS_PACKAGE_PATH,
...
export function ArmUrdfViewer({
  ...
  React.useEffect(() => {
    ...
    loader.packages = modelCfg.packages || { [modelCfg.packageName]: modelCfg.packagePath };
    loader.load(
      modelCfg.urdfPath,
      (robot) => {
